...
- [x] **3. 카드 클릭 전환 + 세션 유지 실측** — 카드 클릭 전환 → `claude` 새 세션 시작 → `/status`에서 전환된 계정 확인. ★ 세션 유지 실측: 전환 전 실행해둔 claude 세션에서 계속 대화 시도 → 결과(무중단/재시작 필요)를 README에 기록. *(사용자 수동 확인 2026-07-11. 세션 유지 실측: 실행 중이던 claude 세션이 수동/자동 전환 왕복을 여러 차례 겪고도 무중단으로 계속 동작 — 단, 실행 중 세션은 이미 로드한 자격증명을 쓰므로 새 계정 적용은 세션 재시작 필요. README에 기록함)*
