...
21. **익명 로그 라인의 귀속 규칙을 신호 종류마다 따로 적용해 절반만 지켜짐 (이슈 #19, 외부
    ...
    **기록**은 그 밖에 있었다. 결정과 기록이 다른 게이트 아래 있으면 절반만 지켜진다(#14와
    ...
    ★ **모델 전용 한도(weekly_scoped/Fable)를 검증에 넣었다가 되돌렸다** — "그것만으로 막힌
    소진이 창 여유로 버려진다"는 이유로 넣었는데, **이 이슈를 더 나쁘게 재현시킨다**: 모델
    한도 100%는 며칠 유지되는 **상태**라 익명 로그 라인의 귀속 증거가 못 되는데,
    `AccountProfile.isLimited`가 modelScoped를 구분하지 않으므로 멀쩡한 폴백이 **며칠**
    후보에서 빠진다(hours → days). 모델 스코프 기반 전환을 하려면 **isLimited/firstAvailable이
    modelScoped를 이해하도록 먼저 고쳐야** 한다 — 메뉴바·CLI·게이지까지 걸치는 별도 변경.
