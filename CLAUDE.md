...
21. **익명 로그 라인의 귀속 규칙을 신호 종류마다 따로 적용해 절반만 지켜짐 (이슈 #19, 외부
    ...
    **기록**은 그 밖에 있었다. 결정과 기록이 다른 게이트 아래 있으면 절반만 지켜진다(#14와
    ...
    죽이지만, 미룬 기록은 다음 hit에서 다시 잡힌다(소진 중엔 에러가 계속 온다).
    부수 효과로 리셋 시각이 정확해진다(로그 "resets 9pm"=21:00 → API 20:59:59).
    ★ **검증 캐시는 표시용(4분)보다 짧게(60초)** — 4분 전 스냅샷이 95%였는데 지금 100%면
    진짜 소진을 "여유"로 오판해 사용자가 막힌 채로 그만큼 더 기다린다.
    ★ **계정 창(5h/7d)만 보면 반대 방향으로 같은 버그** — `UsageSnapshot.exhaustionHit`이
    모델 전용 한도(`limits[].weekly_scoped`, Fable)를 안 봐서, 그것만으로 막힌 소진이
    "창 여유"로 버려진다 → `scopedExhaustionHit` 추가(modelScoped=true라 pin을 존중).
